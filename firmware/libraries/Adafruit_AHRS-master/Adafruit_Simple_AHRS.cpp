...
// Create a simple AHRS from an explicit accelerometer and magnetometer sensor AND GYRO.
...
// Compute orientation based on accelerometer and magnetometer data.
bool Adafruit_Simple_AHRS::getQuad(sensors_vec_t* orientation) {
  // Validate input and available sensors.
  if (orientation == NULL || _accel == NULL || _mag == NULL || _gyro == NULL) return false;
  ...
  orientation->g_x = gyro_event.gyro.x;
  orientation->g_y = gyro_event.gyro.y;
  orientation->g_z = gyro_event.gyro.z;
